...
title = "HashiCorp Vault Data Protection Demo"
...
def home():
  ...
  global ccs
  global card_delimiter
  return render_template('index.html', title=title, ccs=ccs, card_delimiter=card_delimiter)

@app.route("/gencc", methods=["POST"])
def gencc():
  global ccs
  global card_delimiter
  # capture delimiter
  if "delimiter" in request.form:
    card_delimiter = request.form["delimiter"]
  # add cards to working sample
  if "card_count" in request.form:
    ...
    if card_count == 0:
      ...
      for i in range(card_count):
        ...
        ccs[card] = content
  # transit encrypt
  if "transit" in request.form:
    transit_request = request.form["transit"]
    if transit_request == "encrypt":
      cards_from_form = request.form.getlist('card')
      for card in cards_from_form:
        ciphertext = transit_encrypt(card)
        fpe = ccs[card]['fpe']
        token = ccs[card]['token']
        ccs[card] = {'card': card, 'ciphertext': ciphertext, 'fpe': fpe, 'token': token}
  # transform fpe
  if "transform" in request.form:
    transform_request = request.form["transform"]
    if transform_request == "encode":
      cards_from_form = request.form.getlist('card')
      ...
      for card in cards_from_form:
        ciphertext = ccs[card]['ciphertext']

        fpe = transform_encode(
          ccs[card]['card'],
          vault_fpe_mount, 
          vault_fpe_role,
          vault_fpe_transformation,
  ...
  # transform tokenize
  if "tokenize" in request.form:
    tokenize_request = request.form["tokenize"]
    if tokenize_request == "encode":
      cards_from_form = request.form.getlist('card')
